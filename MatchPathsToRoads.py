...
road_segment_lenght = (
    150  # Maximum segment lenght for created segments for the road network
)
...
# Input data
roads_url = "D:/OneDrive - Trivector AB/Projekt/Cykel mellan tatorter/Spring 2021/Test new alog/data/Roads_test_sample.gpkg|layername=Roads_test_sample"

if sample_size == 'large':
    paths_url = "D:/OneDrive - Trivector AB/Projekt/Cykel mellan tatorter/Spring 2021/Test new alog/data/Test_sample_bike_paths_all.gpkg|layername=Test_sample_bike_paths_all"
else:
    paths_url = "D:\OneDrive - Trivector AB\Projekt\Cykel mellan tatorter\Spring 2021\Test new alog\data\Test_sample_bike_paths_small.gpkg|layername=Test_sample_bike_paths_small"
...
def delete_fields_from_layer(vlayer):
    fields_to_keep = {
        ...
        'PathID',
        field_to_summarize,
    }  # set with field names to keepLayerSet
...
def add_data_to_road_segments(buffer_dict, outputs):
    ...
    for segment in road_segments.getFeatures():
        if segment['SegmentID'] in sum_segments:
            road_segments.changeAttributeValue(
                segment.id(), road_segments_sum_idx, sum_segments[segment['SegmentID']]
            )
...
def match(outputs):
    ...
    for path in paths:
        ...
        # Points with index
        alg_params = {'INPUT': outputs['PointsAlongGeometry']['OUTPUT']}
        outputs['PointsAlongGeometryWithIndex'] = processing.run(
        ...
        outputs['JoinAttributesByLocation'] = processing.run(
            'native:joinattributesbylocation', alg_params
        )
...
def main():
    ...
    outputs['ExtractLayerExtent'] = processing.run(
    ...
    alg_params = {'INPUT': outputs['Buffer']['OUTPUT']}
    ...
    outputs['paths_layer']['OUTPUT'] = delete_fields_from_layer(
